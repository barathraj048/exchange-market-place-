...
async function seed() {
  ...
  let tradePromises: (() => Promise<void>)[] = [];
...
async function executeBatched(requests: (() => Promise<void>)[], batchSize: number) {
  for (let i = 0; i < requests.length; i += batchSize) {
    const batch = requests.slice(i, i + batchSize);
    await Promise.all(batch.map(req => 
      req().catch((e: any) => console.log(`Warning: ${e.response?.data?.message || e.message}`))
    ));
